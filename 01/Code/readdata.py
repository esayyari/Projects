...
def LR_SGD_grid(X, y, bsize=1,fold=4, range_eta=[1e-4, 1e-3, 1e-2, 1e-1, 1e0, 1e1]):
    best_acc, best_eta=0, None
    with open(str(bsize)+'LR.SGD.grid', 'w') as f:
        for eta in range_eta:
            acc=cross_validation(X, y, bsize, fold, eta, "SGD")
            if acc>best_acc:
    ...
    return best_eta, best_acc
            

def RLR_SGD_grid(X, y, bsize=1, fold=4, range_weight_decay=[1e-2, 1e-1, 1e0, 1e1, 1e2], range_eta= [1e-3, 1e-2, 1e-1, 1e0, 1e1]):
# def RLR_SGD_grid(X, y, fold=4, range_weight_decay=[1e2], range_eta= [1e-1, 1e0, 1e1]):
    best_acc, best_eta, best_weight=0, None, None
    with open(str(bsize)+'RLR.SGD.grid', 'w') as f:
        for eta in range_eta:
            for weight in range_weight_decay:
                acc=cross_validation(X, y, bsize, fold, eta, "SGD", weight)
                if acc>best_acc:
    ...
    return best_eta, best_weight, best_acc
